...
const processFile = function(filename, width, content) {
    ...
    if (_metadata) {
        ...
        _metadata.content = (_metadata.content.length > 0) ? DOMPurify.sanitize(_metadata.content, {ALLOWED_TAGS: ['img', '#text'], SAFE_FOR_TEMPLATES: true}) : _metadata.content;

        delete _metadata['__content'];

        // map user-entered date to a better one using moment's great parser
        if(_metadata.date) _metadata.iso8601Date = moment(_metadata.date).format();
